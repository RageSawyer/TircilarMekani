...
public DurumAyarla()
{
	...
	{
	    StartRecordingPlayback(PLAYER_RECORDING_TYPE_DRIVER,"SF_LV_D");
	    DurumDegistir(DURUM_S1_D);
	}else if(ODurum == DURUM_S1_D)
	{
		StartRecordingPlayback(PLAYER_RECORDING_TYPE_DRIVER,"SF-LV");
		DurumDegistir(DURUM_S1_S2);
	}else if(ODurum == DURUM_S1_S2)
	{
	    StartRecordingPlayback(PLAYER_RECORDING_TYPE_DRIVER,"LV_SF_D");
	    DurumDegistir(DURUM_S2_D);
	}else if(ODurum == DURUM_S2_D)
	{
		StartRecordingPlayback(PLAYER_RECORDING_TYPE_DRIVER,"LV-SF");
...
public DurumDegistir(durumid)
{
	...
	if(fexist(DosyaIsmi))
	{
	    ...
	    while(fread(Dosya,DigerDosyaYazi,256))
	    {
	        ...
	        if(!strcmp("LS_LV",ini_GetKey(DigerDosyaYazi),true)) { format(DosyaYazi,256,"%sLS_LV=%d\r\n",DosyaYazi,strval( ini_GetValue(DigerDosyaYazi) )); }
	        if(!strcmp("SF_LV",ini_GetKey(DigerDosyaYazi),true)) { format(DosyaYazi,256,"%sSF_LV=%d",DosyaYazi,durumid); }
		}
